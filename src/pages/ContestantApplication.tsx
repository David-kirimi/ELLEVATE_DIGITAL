...
import { Trophy, Send, User, Music, Camera, Link as LinkIcon, AlertCircle, CheckCircle } from 'lucide-react';
import { db, auth } from '../firebase';
import { collection, addDoc, serverTimestamp, doc, getDoc, getDocs, query, orderBy } from 'firebase/firestore';

export default function ContestantApplication() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (userDoc.exists()) {
        ...
        if (userDoc.data().role === 'contestant') {
          setError("You are already a contestant!");
        }
      }

      // Fetch dynamic questions
      const q = query(collection(db, 'applicationQuestions'), orderBy('order', 'asc'));
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-6 pt-20">
        ...
        >
          ...
          <p className="text-gray-500 mb-10">
            Thank you for applying to be a contestant. Our team will review your application and get back to you soon.
          </p>
          <button 
      ...
      <div className="max-w-3xl mx-auto">
        <div className="text-center mb-12">
          <div className="inline-flex items-center space-x-2 text-brand-orange mb-4">
            ...
            <span className="text-sm font-bold uppercase tracking-widest">Contestant Program</span>
          </div>
          <h1 className="text-4xl md:text-5xl font-bold mb-6">Apply to Compete</h1>
        ...
        >
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-3">Proof of Talent (Link)</label>
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full pl-12 pr-6 py-4 rounded-2xl bg-gray-50 border-none focus:ring-2 focus:ring-brand-orange outline-none"
                  placeholder="YouTube, Instagram, or Portfolio link"
                />
              </div>
              <p className="mt-2 text-xs text-gray-400">Share a link to a video or page that showcases your talent.</p>
            </div>
